...
# # --- 1. Läs in din CSV-fil ---
# df = pd.read_csv("data/studerande_ren.csv")

# # --- 2. Omvandla från bred till lång format ---
# # Vi antar att första kolumnen heter 'År'
# df_long = df.melt(id_vars=["År"], var_name="Utbildningsområde", value_name="Antal")

# # --- 3. Rensa och konvertera 'Antal' till numeriskt ---
# df_long["Antal"] = (
#     df_long["Antal"]
#     .astype(str)
#     .str.replace(r"\s+", "", regex=True)   # Ta bort mellanslag
#     .str.replace(",", ".")                 # Byt komma mot punkt om decimaler finns (inte säkert för antal)
#     .replace("..", None)                   # Hantera saknade värden
# )
# df_long["Antal"] = pd.to_numeric(df_long["Antal"], errors="coerce")

# # --- 4. Konvertera 'År' till int ---
# df_long["År"] = df_long["År"].astype(int)

# # --- 5. Skapa lista med år för dropdown ---
# available_years = [str(y) for y in sorted(df_long["År"].unique())]
# selected_year = available_years[0]  # Förvalt valt år

# # --- 6. Funktion för horisontell stapeldiagram ---
...
# # --- 7. Funktion för att uppdatera graf ---
